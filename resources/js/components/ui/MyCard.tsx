...
export default function MyCard({title, bgColor, children, disableHeaderPadding, disablePadding, action, ...props}: MyCardProps) {
    ...
    return (
        <Card sx={{display: 'flex', flexDirection: 'column'}} {...props}>
            ...
            {title && (<CardHeader title={title} sx={{borderLeft: `10px solid ${bgColor}`, headerPadding}} action={action}/>)}
