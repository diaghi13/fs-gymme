...
import { centralMenuList } from '@/layouts/index';
...
export default function CentralLayout({ title, children, user }: PropsWithChildren<{ user: User, title?: string, }>) {
  ...
  useEffect(() => {
    ...
    if (page.props.flash) {
      try {
        const flashData = JSON.parse(JSON.stringify(page.props.flash));
        setStatus(flashData.status || null);
        setMessage(flashData.message || null);
        setOpenAlert(!!flashData.status);
      } catch (error) {
        console.error("Error cloning flash data:", error);
        setStatus(null);
        setMessage(null);
        setOpenAlert(false);
      }
